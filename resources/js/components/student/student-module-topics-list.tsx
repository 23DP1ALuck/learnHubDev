...
export default function StudentModuleTopicsList({
    moduleId,
    topics,
}: StudentModuleTopicsListProps) {
    ...
    return (
        <Card className="border-sidebar-border/70">
            <CardHeader>
                ...
                <CardDescription>
                    {t('learning.Topics available inside this module.')}
                </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
                {topics.length === 0 ? (
                    <div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">
                        {t('student.No topics are available yet.')}
                    </div>
                ) : (
                    topics.map((topic) => (
                        <Link
                        ...
                        >
                            ...
                            <div className="mt-3 flex flex-wrap gap-3 text-xs text-muted-foreground">
                                <span>
                                    {topic.assignments_count} assignments
                                </span>
                                <span>
                                    {t('student.Topic #')}
                                    {topic.topic_id}
