...
export default function StudentTaskSidebar({
    assignment,
    task,
    taskNavigation,
}: StudentTaskSidebarProps) {
                ...
                <CardContent className="grid gap-4">
                    <div className="rounded-2xl border bg-muted/30 p-4">
                        <p className="text-3xl font-semibold tracking-tight">
                            {dueStateLabel(assignment.due_date)}
                        </p>
                    </div>
            ...
            <Card className="border-sidebar-border/70">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-lg">
                        ...
                        {t('student.Task navigation')}
                    </CardTitle>
                    <CardDescription>
                        {t(
                            'student.Move between tasks in the same assignment.',
                        )}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-4 gap-3">
                        {taskNavigation.map((item) => {
                            ...
                            return (
                                <Link
                                    key={item.task_id}
                                    className={`flex aspect-square items-center justify-center rounded-2xl border text-base font-semibold transition ${isCurrent ? 'border-primary bg-primary text-primary-foreground shadow-sm' : item.is_completed ? 'bg-muted-foreground/20 hover:border-primary/40' : 'bg-background hover:border-primary/40 hover:bg-muted/40'}`}
