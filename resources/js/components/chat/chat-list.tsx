    ...
    return (
        <Card className="border-sidebar-border/70">
            <CardHeader className="gap-3">
                <div>
                    <CardTitle>{t('chat.Chats')}</CardTitle>
                    <CardDescription>
                        {t('chat.Private, group, and module conversations.')}
                    </CardDescription>
                </div>
                <div className="relative">
                    ...
                    <Input
                        className="pl-9"
                        placeholder={t('chat.Search chats')}
                    />
                </div>
            </CardHeader>
            <CardContent className="space-y-3">
                {chats.length === 0 ? (
                    <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">
                        {t('chat.No chats are available yet.')}
                    </div>
                ) : (
                    chats.map((chat) => (
                        <Link
                        ...
                        >
                            ...
                            {chat.participants_preview.length > 0 ? (
                                <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
                                    ...
                                    {chat.participants_preview.length > 3 ? (
                                        <span>
                                            +
                                            {chat.participants_preview.length -
                                                3}{' '}
                                            more
                                        </span>
                                    ) : null}
