...
function MessageBubble({ message }: { message: ChatMessage }) {
    return (
        ...
        >
            <div
                className={`max-w-2xl rounded-3xl px-4 py-3 ${message.is_mine ? 'bg-primary text-primary-foreground' : 'border bg-card'}`}
            >
...
function getXsrfToken(): string {
    // need for laravel POST request if make request via fetch
...
export default function ChatThread({ activeChat }: ChatThreadProps) {
    ...
    if (!activeChat) {
        return (
            <Card className="border-sidebar-border/70">
                <CardHeader>
                    <CardTitle>{t('chat.Open a chat')}</CardTitle>
                    <CardDescription>
                        {t(
                            'chat.Select a chat from the list or create a new one to start messaging.',
                        )}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="rounded-3xl border border-dashed p-12 text-center text-sm text-muted-foreground">
                        {t(
                            'chat.The conversation area will show messages, participants, and the composer once a chat is selected.',
                        )}
                    </div>
                </CardContent>
            </Card>
    ...
    return <ActiveChatThread key={activeChat.id} activeChat={activeChat} />;
}
function ActiveChatThread({ activeChat }: { activeChat: ActiveChat }) {
    const { t } = useTranslation();
    ...
    useEffect(() => {
        // scroll to bottom of messages list if new message received
    ...
    useEcho(
        ...
        (event: IncomingChatMessageEvent) => {
            if (
                !activeChat ||
                event.chat_id !== activeChat.id ||
                event.sender_id === auth.user.id
            ) {
                return;
            }
    ...
    const submitMessage = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await fetch(
                route('chats.messages.store', activeChat.id, false),
                {
                    method: 'POST',
    ...
    return (
        <Card className="border-sidebar-border/70 py-2">
                ...
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        {activeChat.unread_count > 0 ? (
                            <Badge>{activeChat.unread_count} unread</Badge>
                        ) : null}
                    </div>
                </div>
            </CardHeader>
            <CardContent className="grid h-[68dvh] grid-rows-[minmax(0,1fr)_auto] gap-4 p-4">
                <div
                    className="space-y-3 overflow-y-auto rounded-3xl bg-muted/20 p-3"
                    ref={messagesListRef}
                >
                    {messages.length === 0 ? (
                        <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">
                            {t(
                                'chat.No messages yet. The first message in this chat will appear here.',
                            )}
                        </div>
                    ) : (
                ...
                >
                    ...
                    <textarea
                        ...
                        onKeyDown={(event) =>
                            sendViaShiftEnter(event, sendButtonRef)
                        }
