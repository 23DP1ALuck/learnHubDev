...
export default function TaskPreviewSidebar({
    assignment,
    task,
    taskNavigation,
            ...
            <Card className="border-sidebar-border/70">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-lg">
                        <Clock3 className="size-4" /> {t('teacher.Time left')}
                    </CardTitle>
            ...
            <Card className="border-sidebar-border/70">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-lg">
                        ...
                        {t('teacher.Task navigation')}
                    </CardTitle>
                    <CardDescription>
                        {t(
                            'teacher.Preview the rest of the assignment from this task grid.',
                        )}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-4 gap-3">
                        {taskNavigation.map((item) => {
                            ...
                            return (
                                <Link
                                    key={item.task_id}
                                    className={`flex aspect-square items-center justify-center rounded-2xl border text-base font-semibold transition ${isCurrent ? 'border-primary bg-primary text-primary-foreground shadow-sm' : 'bg-background hover:border-primary/40 hover:bg-muted/40'}`}
                                    href={route('teacher.tasks.preview', [
                                        assignment.id,
                                        item.task_id,
                                    ])}
                                >
