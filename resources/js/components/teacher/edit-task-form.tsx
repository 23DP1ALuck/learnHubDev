...
export default function EditTaskForm({ assignment, task }: EditTaskFormProps) {
    ...
    >(
        task.task_type === 'CHECKBOX'
            ? task.correct_answers.map((answer) => answer.answer_id)
            : [],
    );
    ...
    const resetTaskBuilder = () => {
        ...
        nextOptionId.current =
            task.options.length > 0
                ? Math.max(...task.options.map((option) => option.option_id)) +
                  1
                : 3;
    };
    const addOption = () => {
    ...
    return (
        <Card className="border-sidebar-border/70">
            <CardHeader>
                <CardTitle>{t('teacher.Edit task')}</CardTitle>
                <CardDescription>
                    {t('teacher.Modify information below to update the task.')}
                </CardDescription>
            </CardHeader>
