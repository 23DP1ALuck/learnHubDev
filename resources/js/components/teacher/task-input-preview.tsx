    ...
    switch (task.task_type) {
        case 'TEXT':
            return (
                <div className="rounded-2xl border border-dashed bg-muted/30 p-4">
                    <p className="mb-3 text-sm font-medium text-muted-foreground">Student answer</p>
                    <textarea
        ...
        case 'NUMBER':
            return (
                <div className="rounded-2xl border border-dashed bg-muted/30 p-4">
                    <p className="mb-3 text-sm font-medium text-muted-foreground">Student answer</p>
                    <Input disabled placeholder="Students will enter a numeric answer." type="number" />
                </div>
            );
        ...
        case 'FILE':
            return (
                <div className="rounded-2xl border border-dashed bg-muted/30 p-6 text-center">
                    ...
                    <p className="font-medium">File upload task</p>
                    <p className="mt-2 text-sm text-muted-foreground">Students will upload one or more files here. This task is reviewed manually.</p>
                </div>
            );
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                    {['TRUE', 'FALSE'].map((value) => (
                        <label key={value} className="flex items-center gap-3 rounded-2xl border p-4 opacity-80">
                            <input disabled name="preview-true-false" type="radio" />
            ...
            return (
                <div className="space-y-3">
                    {options.length === 0 ? (
                        ...
                        options.map((option) => (
                            <label key={option.option_id} className="flex items-center gap-3 rounded-2xl border p-4 opacity-80">
                                <input disabled name="preview-custom-select" type="radio" />
                                <span>{option.option_text}</span>
                            </label>
                        ))
                    )}
