...
export default function TeacherDashboardAssignments({
    assignments,
}: TeacherDashboardAssignmentsProps) {
    const { t } = useTranslation();
    return (
        <Card className="border-sidebar-border/70">
            <CardHeader>
                <CardTitle>{t('learning.Upcoming assignments')}</CardTitle>
                <CardDescription>
                    {t(
                        'teacher.Assignments ordered for quick access from the dashboard.',
                    )}
                </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
                {assignments.length === 0 ? (
                    <div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">
                        {t('teacher.No assignments are available yet.')}
                    </div>
                ) : (
