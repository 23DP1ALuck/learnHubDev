...
export default function TopicAssignmentCreate({
    moduleId,
    topicId,
    availableTopics,
}: TopicAssignmentCreateProps) {
    const { t } = useTranslation();
    const [selectedTopicIds, setSelectedTopicIds] = useState<number[]>([]);
    const currentTopic = availableTopics.find(
        (topic) => topic.topic_id === topicId,
    );
    const additionalTopics = availableTopics.filter(
        (topic) => topic.topic_id !== topicId,
    );
    const toggleTopic = (nextTopicId: number, checked: boolean) => {
        setSelectedTopicIds((currentIds) =>
            checked
                ? [
                      ...currentIds.filter((id) => id !== nextTopicId),
                      nextTopicId,
                  ]
                : currentIds.filter((id) => id !== nextTopicId),
        );
    ...
    return (
        <Card className="border-sidebar-border/70">
            <CardHeader>
                <CardTitle>{t('learning.Create assignment')}</CardTitle>
                <CardDescription>
                    {t(
                        'teacher.The current topic is always linked. You can also attach this assignment to additional topics in the same module.',
                    )}
                </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                >
                    {({ processing, errors }) => (
                        <>
                            <input
                                type="hidden"
                                name="topics[0][module_id]"
                                value={moduleId}
                            />
                            <input
                                type="hidden"
                                name="topics[0][topic_id]"
                                value={topicId}
                            />
                            {selectedTopicIds.map((selectedTopicId, index) => (
                                <div key={selectedTopicId} className="hidden">
                                    <input
                                        type="hidden"
                                        name={`topics[${index + 1}][module_id]`}
                                        value={moduleId}
                                    />
                                    <input
                                        type="hidden"
                                        name={`topics[${index + 1}][topic_id]`}
                            ...
                            <div className="grid gap-2">
                                <Label htmlFor="assignment-title">
                                    {t('learning.Assignment title')}
                                </Label>
                                <Input
                                    id="assignment-title"
                                    name="title"
                                    placeholder={t('teacher.Homework 1')}
                                />
                            ...
                            <div className="grid gap-2">
                                ...
                                <textarea
                                    ...
                                    placeholder={t(
                                        'teacher.Explain what students need to submit or answer.',
                                    )}
                                />
                            ...
                            <div className="grid gap-3">
                                <div className="grid gap-1">
                                    <Label>{t('learning.Linked topics')}</Label>
                                    <p className="text-sm text-muted-foreground">
                                        {t(
                                            'teacher.This topic is required. Add more topics if the assignment covers them too.',
                                        )}
                                    </p>
                                </div>
                                <div className="rounded-md border px-3 py-2 text-sm">
                                    <span className="font-medium">
                                        {t('teacher.Current topic')}
                                    </span>
                                    : {currentTopic?.name ?? `Topic ${topicId}`}
                                </div>
