...
export default function TeacherDashboardModules({
    modules,
}: TeacherDashboardModulesProps) {
    const { t } = useTranslation();
    return (
        <Card className="border-sidebar-border/70">
            <CardHeader>
                <CardTitle>{t('learning.Recent modules')}</CardTitle>
                <CardDescription>
                    {t(
                        'teacher.The latest modules in your teaching workspace.',
                    )}
                </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
                {modules.length === 0 ? (
                    <div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">
                        {t('teacher.No modules are available yet.')}
                    </div>
                ) : (
                    modules.map((module) => (
                        <Link
                        ...
                        >
                            ...
                            <div className="mt-3 flex flex-wrap gap-3 text-xs text-muted-foreground">
                                ...
                                <span>
                                    {module.start_date || 'No start date'} -{' '}
                                    {module.end_date || 'No end date'}
                                </span>
                            </div>
                        </Link>
                    ))
